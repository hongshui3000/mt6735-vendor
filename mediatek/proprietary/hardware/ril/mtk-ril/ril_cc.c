...
extern bool afterCallConnected;
...
void requestDial(void *data, size_t datalen, RIL_Token t, int isEmergency)
{
    ...
    if (dial_source == IMSPHONE) {
        LOGE("IMS: requestDial from IMS and set afterCallConnected = false");
        call_state = mo;
        afterCallConnected = false;
        pendingSrvcc = false;
...
extern int rilCcMain(int request, void *data, size_t datalen, RIL_Token t)
{
    ...
    case RIL_REQUEST_SET_VT_CAPABILITY:
        requestSetVTCapability(data, datalen, t);
        break;
