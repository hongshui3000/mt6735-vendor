...
static void onRequest(int request, void *data, size_t datalen, RIL_Token t, RIL_SOCKET_ID socket_id)
{
    ...
    if (radioState == RADIO_STATE_OFF &&
            !(request == RIL_REQUEST_RADIO_POWER ||
              ...
              request == RIL_REQUEST_SET_VT_CAPABILITY ||
              // Remote SIM access
              request == RIL_REQUEST_CONFIG_MODEM_STATUS||
    ...
    if (isInternationalRoamingEnabled()
        ...
        if (isDualTalkMode()) {
            ...
            if (rid == RIL_SOCKET_1) {
                ...
                RLOGD("case 0 get SIM inserted status [%d]", sim_inserted_status);
            }
#endif
...
int isCdmaMdIratSupport()
{
    int isMdIratSupport = 0;
    char property_value[PROPERTY_VALUE_MAX] = { 0 };
    property_get("ro.c2k.md.irat.support", property_value, "0");
    isMdIratSupport = atoi(property_value);
    RLOGI("isCdmaMdIratSupport: %d", isMdIratSupport);
    return isMdIratSupport;
}
