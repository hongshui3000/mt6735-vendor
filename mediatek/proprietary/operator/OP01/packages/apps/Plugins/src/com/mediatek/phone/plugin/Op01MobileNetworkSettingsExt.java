...
public class Op01MobileNetworkSettingsExt extends DefaultMobileNetworkSettingsExt {
    ...
    public void initMobileNetworkSettings(PreferenceActivity activity, int currentTab) {
        ...
        if (activity != null) {
            try {
                mContext = activity.createPackageContext("com.mediatek.op01.plugin",
    ...
    private boolean isLTEModeEnable(int slotId, Context context) {
        ...
        if (SIM.equals(type) ||
                (!isSupportWorldPhonePolicy() && isCUOrCTCard(slotId))) {
            log("isLTEModeEnable, SIM card, or not WorldPhonePolicy CU CT card, shoule be disabled");
            return false;
        }
