...
import com.mediatek.backuprestore.StorageListActivity;
...
public class BackupFileScanner {
    ...
    private class ScanThread extends Thread {
            ...
            if(files != null && files.length > 0) {
                for (File currentFile : files) {
                    ...
                    if (currentFile.exists() && currentFile.isDirectory()) {
                        ...
                        if(checkDataForAdd(currentFile.getAbsolutePath())) {
                        ...
                        if(checkDataForAdd(currentFile.getParentFile().getParentFile().getAbsolutePath())) {
        ...
        private boolean checkDataForAdd(String path) {
            if(mDataResult != null && mDataResult.size() > 0) {
                for(int i = 0; i < mDataResult.size(); i++) {
                    ...
                    if(obj instanceof BackupFilePreview) {
                        ...
                        MyLogger.logE(CLASS_TAG, "checkDataForAdd APP : " + path);
                        if(appData.getAppData().contains(appData)) {
                            MyLogger.logE(CLASS_TAG, "checkDataForAdd : false ");
                            return false;
                        }
