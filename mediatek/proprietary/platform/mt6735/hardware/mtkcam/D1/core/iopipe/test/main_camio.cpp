...
void onDumpBuffer(const char* usr, MUINT32 const data, MUINTPTR const buf, MUINT32 const ext)
{
...
#define DUMP_IImageBuffer( type, pbuf, fileExt )               \
    ...
    {
        case 0:
            DUMP_IImageBuffer( 0, buf, "raw" );
            break;
        case 1:
            DUMP_IImageBuffer( 1, buf, "raw" );
            break;
        default:
   ...
   // printf("!!!!!!!! press enter to stop... !!!!!!!!\n");//*/
    ...
    for(int k=sensorIdx;k<dualSensor;k++){
        for(int i=0;i<Enque_buf;i++){
            if(enablePort & __RRZO_ENABLE){
                //note:crop size can't < outsize
                onDumpBuffer("camio", 1, (MUINTPTR)pImgBuffer[k][__RRZO][i], 0);
            }
            if(enablePort & __IMGO_ENABLE){
                onDumpBuffer("camio", 0, (MUINTPTR)pImgBuffer[k][__IMGO][i], 0);
            }
        }
    }
    //in this example, dummy buf with mag : 4,5,6
