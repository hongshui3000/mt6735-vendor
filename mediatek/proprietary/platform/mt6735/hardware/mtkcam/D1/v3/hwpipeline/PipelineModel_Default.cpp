    ...
    , mpResMgrDrv(NULL)
{
    ...
    mpResMgrDrv = ResMgrDrv::createInstance(openId);
    if (mpResMgrDrv) {
...
)   const
{
    ...
    MUINT sensorFps_10x[sizeofmode] = {
                             ...
                             sensorStaticInfo.videoFrameRate,
...
#warning"[FIXME] should not always enable img2o"
        ...
        {
            MINT32 const width_min = ALIGN_2(sensorSize.w * P1_SCALE_RATIO_10X / 10);
...
#warning"[FIXME] should enable img2o resize"
            ...
            MINT32 size_w = MAX(
                ALIGN_2(maxYuvStreamSize.w * P2_YUV_EIS_RATIO_10X / 10),
                ALIGN_2((sensorSize.w + 9) * P1_RRZO_SCALE_RATIO_10X / 10));
            MINT32 size_h = MAX(
                ALIGN_2(maxYuvStreamSize.h * P2_YUV_EIS_RATIO_10X / 10),
                ALIGN_2((sensorSize.h + 9) * P1_RRZO_SCALE_RATIO_10X / 10));
...
)
{
    ...
    if (mpResMgrDrv)
    {
        ResMgrDrv::MODE_STRUCT mode;
        mpResMgrDrv->getMode(&mode);
        mode.scenSw = (mHasVRConsumer) ?
                        ResMgrDrv::SCEN_SW_VIDEO_VSS :
                        ResMgrDrv::SCEN_SW_ZSD;
        mpResMgrDrv->setMode(&mode);
    }
