...
class localFuncInOut
{
    public:
        localFuncInOut(char const* const str) : mStr(str) { CAM_LOGD("[%s] +", mStr.string()); }
        ~localFuncInOut() { CAM_LOGD("[%s] -", mStr.string()); }
    private:
        const String8 mStr;
};
#define FUNC_IN_OUT    localFuncInOut localFuncIO(__FUNCTION__)
//
...
{
    CAM_TRACE_NAME("p2");
    localFuncInOut log("P2Processor::onExecute");
    //
    MERROR ret = OK;
    //
...

    {
        ...
        if( pMeta_OutHal ) {
            ...
            for(int i=0; i<pMeta_InHal->count(); i++)
            {   
                if (pMeta_InHal->entryAt(i).tag()==MTK_3A_EXIF_METADATA)
                    MY_LOGD("Metadata InHal-%d: %d", i, pMeta_InHal->entryAt(i).tag() );
            }
        }
        //
...
) const
{
    if( ! tryGetMetadata<MSize>(inHal, MTK_HAL_REQUEST_SENSOR_SIZE, cropInfos.sensor_size) ) {
        MY_LOGE("cannot get MTK_HAL_REQUEST_SENSOR_SIZE => 4192x3104");
		cropInfos.sensor_size = MSize(4192,3104);
        //return BAD_VALUE;
    }
	else
	{
		MY_LOGD("MTK_HAL_REQUEST_SENSOR_SIZE: %dx%d", cropInfos.sensor_size.w, cropInfos.sensor_size.h);
	}
    //
    ...
    // query EIS result
    {
        ...
        if( isEisOn && queryEisRegion(inHal, eisInfo)) {
            ...
            pMv_s->p.x  = ((sensor.w - eisInfo.s.w) >> 1) - eisInfo.x_int;
            pMv_s->pf.x = eisInfo.x_float;
            pMv_s->p.y  = ((sensor.h - eisInfo.s.h) >> 1) - eisInfo.y_int;
            ...
            pMv_r->p.x  = ((resizer.w - eisInfo.s.w) >> 1) - eisInfo.x_int;
            pMv_r->pf.x = eisInfo.x_float;
            pMv_r->p.y  = ((resizer.h - eisInfo.s.h) >> 1) - eisInfo.y_int;
...
{
    CAM_TRACE_NAME("mdp");
    localFuncInOut log("MDPProcessor::onExecute");
    //
    MERROR ret = OK;
    //
