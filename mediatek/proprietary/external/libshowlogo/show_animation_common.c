...
void fill_animation_battery_new(unsigned int capacity, void *fill_addr, void * dec_logo_addr, void * logo_addr, LCM_SCREEN_T phical_screen)
{
  ...
  static unsigned char index30=0;
   static unsigned char index60=0;
  static unsigned char index90=0;
    if (capacity >= 100) {
        //show_logo(37); // battery 100
       // fill_animation_logo(FULL_BATTERY_INDEX, fill_addr, dec_logo_addr, logo_addr,phical_screen);
      fill_animation_logo(50, fill_addr, dec_logo_addr, logo_addr,phical_screen);
    } else if (capacity < 10) {
        LOG_ANIM("[show_animation_common: %s %d]charging_low_index = %d\n",__FUNCTION__,__LINE__, charging_low_index);  
      
           fill_animation_logo(51 + charging_low_index, fill_addr, dec_logo_addr, logo_addr,phical_screen);
	  charging_low_index ++ ;	   
      ...
      //  fill_animation_dynamic(NUMBER_PIC_PERCENT, percent_location_rect, fill_addr, percent_pic_addr, logo_addr, phical_screen);
        ...
        if (charging_low_index >= 4) charging_low_index = 0;
	 ...
	 if (capacity <= 30)
	 {
		   fill_animation_logo(38+index30, fill_addr, dec_logo_addr, logo_addr,phical_screen);	
		   index30++;
		  if(index30>=4)index30=0;
	 }
	 else if(capacity > 30&&capacity <= 60)
	 {
		 fill_animation_logo(42+index60, fill_addr, dec_logo_addr, logo_addr,phical_screen);	
		 index60++;
		 if(index60>=4)index60=0;

	 }
	  else if(capacity > 60&&capacity <100)
	 {
		  fill_animation_logo(46+index90, fill_addr, dec_logo_addr, logo_addr,phical_screen);	 
		  index90++;
	 	 if(index90>=4)index90=0;

	 }
#if 0	  
        //background 
       // fill_animation_logo(ANIM_V1_BACKGROUND_INDEX, fill_addr, dec_logo_addr, logo_addr,phical_screen);
	 fill_animation_logo(5+charging_animation_index, fill_addr, dec_logo_addr, logo_addr,phical_screen);	
